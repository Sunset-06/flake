...
void operate()
{
    uint8_t nibble = opcode & 0xF000u;  //opcode is the current instruction. It gets updated in fetch()
    ...
    switch (nibble)
    {
        ...
        case 0x8000:
            switch (fourthNibble)
            {
                ...
                case 0x0007:
                    registers[0xF] = (registers[thirdNibble] > registers[secondNibble]) ? 1 : 0;
                    registers[secondNibble] = registers[thirdNibble] - registers[secondNibble];
                    break;
                // 0x800E: Shift Vx left by 1
                case 0x000E:
